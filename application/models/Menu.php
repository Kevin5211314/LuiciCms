...
{   
     ...
     * @param    string                   $searchParam [description]
     * @return   [type]                                [description]
     */
    public function get_menu_list($page, $limit, $searchParam = '')
    {   
        if ($searchParam) {
            $query = $this->db->select("*")->from("menu")->limit($limit, $page)->like($searchParam)->get();
